...
import mindly.domain.User;
import mindly.repository.EntryRepository;
import mindly.repository.UserRepository;
import mindly.security.SecurityUtils;
import mindly.service.UserService;
import mindly.web.rest.errors.BadRequestAlertException;
...
/**
 * REST controller for managing {@link mindly.domain.Entry}.
 */
@RestController
@RequestMapping("/api")
public class EntryResource {
    ...
    private boolean bitfinexSuccess = true;
     ...
     */
    @GetMapping("/entries")
    public ResponseEntity<List<Entry>> getAllEntries(Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder) {
        log.debug("REST request to get a page of Entries");
        Page<Entry> page = entryRepository.findAll(pageable);

        Float marketBTC = updateMarketValue("btceur");
        Float marketETH = updateMarketValue("etheur");
        Float marketXRP = marketBTC * updateMarketValue("xrpbtc");

        if (bitfinexSuccess) {
            for (Entry entry : page) {
                switch (entry.getCurrency().name()) {
                    case "Bitcoin":
                        entry.setCurrentPrice(marketBTC * entry.getAmount());
                        break;
                    case "Ethereum":
                        entry.setCurrentPrice(marketETH * entry.getAmount());
                        break;
                    case "Ripple":
                        entry.setCurrentPrice(marketXRP * entry.getAmount());
                        break;
                }
            }
        }
    ...
    private float updateMarketValue(String tickerPair) {
        try {
            URL url = new URL("https://api.bitfinex.com/v1/pubticker/"+tickerPair);
            ...
            if(matcher.find()) {
                bitfinexSuccess = true;
                return Float.parseFloat(matcher.group(1));
            } else {
                bitfinexSuccess = false;
                return 0;
            }
        } catch (IOException e) {
            bitfinexSuccess = false;
